...
public class BoxNetService {
    ...
    private String newTicketUrl = "https://www.box.com/api/1.0/rest?action=get_ticket&api_key=" + apiKey;
    ...
    public BoxNetData.BoxNetTicketResponse getNewTicket() throws Exception {
        ...
        response = (BoxNetData.BoxNetTicketResponse) readXml(
                new BufferedReader(new InputStreamReader(url.openStream())), BoxNetData.BoxNetTicketResponse.class);
        return response;
    }
    ...
    public BoxNetData.BoxNetAuthResponse getAuthToken(BoxNetData.BoxNetTicketResponse ticket) throws Exception {
        String authUrl = "https://www.box.com/api/1.0/rest?action=get_auth_token&api_key=" + apiKey + "&ticket="
                + ticket.getTicket();
        URL url = new URL(authUrl);
        BoxNetData.BoxNetAuthResponse authResponse = (BoxNetData.BoxNetAuthResponse) readXml(new BufferedReader(
                new InputStreamReader(url.openStream())), BoxNetData.BoxNetAuthResponse.class);
        return authResponse;
    }
    ...
    public BoxNetFileUploadResponse saveFileToBoxNet(SaveFileData saveFileData) {
        ...
        // return
        // 'a file with that id already exists' and then we need to hit it again
        // with
        ...
        try {
            for (int i = 0; i < 2; ++i) {
                ...
                if (boxNetResponse.entries.size() > 0) {
                    ...
                    if (entry.type.equalsIgnoreCase("error") && entry.status.equalsIgnoreCase("409")
                            && entry.context_info.conflicts.size() > 0) {
                        BoxNetData.BoxNetFileUploadConflicts conflicts = entry.context_info.conflicts.get(0);
                        // Copy the correct file id again and retry one more
                        // time.
    ...
    /*
     * public static void main(String[] args) throws Exception { BoxNetService
     * service = new BoxNetService(); BoxNetTicketResponse ticket =
     * service.getNewTicket(); BoxNetAuthResponse authToken =
     * service.getAuthToken(ticket); authToken.auth_token =
     * "s7q6hkklutef9ex5jxk4z0kj7bkgcjq6"; BoxNetService boxNetService = new
     * BoxNetService(); BoxNetFileUploadResponse saveResult =
     * boxNetService.saveFileToBoxNet( new SaveFileData(authToken.auth_token,
     * "noname-1.85", "183787071", "This is test data."));
