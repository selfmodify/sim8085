...
public class TestBoxNetService extends TestCase {
    ...
    public void testBoxExchange() throws Exception {
        ...
        String errorResponse = FileUtils.readFileToString(new File("box-net-file-upload-error-response.txt"));
        // read the error response data
        String successResponse = FileUtils.readFileToString(new File("box-net-file-upload-response.txt"));
        SaveFileData saveFileData = new SaveFileData("s7q6hkklutef9ex5jxk4z0kj7bkgcjq6", "noname-1.85", "3011590059",
                "This is test data.3");
        when(boxNet.getFileUploadResponse(saveFileData)).thenReturn(errorResponse, successResponse);
        BoxNetData.BoxNetFileUploadResponse response = boxNet.saveFileToBoxNet(saveFileData);
        assertNotNull(response);
        assertTrue("At least one entry for the uploaded file must exist", response.total_count > 1);
    ...
    public void testCreateBoxNetRequestData() throws Exception {
        ...
        String url = fileId == null ? "https://api.box.com/2.0/files/" + fileId + "/data"
