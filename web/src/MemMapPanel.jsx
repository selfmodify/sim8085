...
export function MemMapPanel({ regs, programRegion, presetAddrs, onJump, onJumpDisasm, onGotoLine, dragHandleProps, dropTargetProps, isDragOver, theme, popoutCrtProps }) {
  ...
  const handleDataClick = () => {
    if (dataRegions.length > 0) {
      ...
      onJump?.(r.start & 0xFFF0); onGotoLine?.(r.start); onJumpDisasm?.(r.start);
        ...
        <div className="panel-anim-body memmap-body" style={poppedOut ? { flex: 1, overflowY: 'auto' } : undefined}>
          <div className="memmap-bar-container">
            <div className="memmap-bar">
              {programRegion && <div className="memmap-region memmap-code" style={{ top: `${(programRegion.start/65535)*100}%`, height: `${Math.max(0.5, ((programRegion.end-programRegion.start)/65535)*100)}%` }} onClick={() => { setSelectedInfo(`Code: ${hex4(programRegion.start)}H - ${hex4(programRegion.end)}H`); onJump?.(programRegion.start & 0xFFF0); onGotoLine?.(programRegion.start); onJumpDisasm?.(programRegion.start); }} />}
              {dataRegions.map((r, i) => <div key={r.start} className="memmap-region memmap-data" style={{ top: `${(r.start/65535)*100}%`, height: `${Math.max(0.5, ((r.end-r.start)/65535)*100)}%` }} onClick={() => { setSelectedInfo(`Data: ${hex4(r.start)}H - ${hex4(r.end)}H`); onJump?.(r.start & 0xFFF0); onGotoLine?.(r.start); onJumpDisasm?.(r.start); }} />)}
              {regs.sp > 0 && <div className="memmap-region memmap-stack" style={{ top: `${(regs.sp/65535)*100}%`, height: `${((65536-regs.sp)/65535)*100}%` }} onClick={() => { setSelectedInfo(`Stack: ${hex4(regs.sp)}H - FFFFH`); onJump?.(regs.sp & 0xFFF0); onGotoLine?.(regs.sp); onJumpDisasm?.(regs.sp); }} />}
              <div className="memmap-marker memmap-pc" style={{ top: `${(regs.pc/65535)*100}%` }} onClick={() => { setSelectedInfo(`PC: ${hex4(regs.pc)}H`); onJump?.(regs.pc & 0xFFF0); onGotoLine?.(regs.pc); onJumpDisasm?.(regs.pc); }} />
            </div>
          ...
          <div className="memmap-legend">
            <div className="memmap-legend-grid">
              <div className="memmap-legend-item" onClick={() => { if (programRegion) { setSelectedInfo(`Code: ${hex4(programRegion.start)}H - ${hex4(programRegion.end)}H`); onJump?.(programRegion.start & 0xFFF0); onGotoLine?.(programRegion.start); onJumpDisasm?.(programRegion.start); } }}><span className="memmap-swatch" style={{background: 'var(--tint-blue-code)', borderColor: 'rgba(64,144,255,.5)'}}/> CODE</div>
              ...
              <div className="memmap-legend-item" onClick={() => { if (regs.sp > 0) { setSelectedInfo(`Stack: ${hex4(regs.sp)}H - FFFFH`); onJump?.(regs.sp & 0xFFF0); onGotoLine?.(regs.sp); onJumpDisasm?.(regs.sp); } }}><span className="memmap-swatch" style={{background: 'var(--tint-amber-sp)', borderColor: 'var(--amber)'}}/> STACK</div>
              <div className="memmap-legend-item" onClick={() => { setSelectedInfo(`PC: ${hex4(regs.pc)}H`); onJump?.(regs.pc & 0xFFF0); onGotoLine?.(regs.pc); onJumpDisasm?.(regs.pc); }}><span className="memmap-swatch" style={{background: 'var(--tint-accent-pc)', borderColor: 'var(--accent)'}}/> PC</div>
            </div>
