export const INST_HELP = {
  ...
  PUSH: { brief:'Push register pair onto stack', flags:'—', bytes:1, cycles:'11', desc:'SP decrements by 2; high byte pushed first, then low byte. PUSH PSW pushes A and flags.', ex:'PUSH B        ; push BC\nPUSH PSW      ; push A + flags' },
  ...
  XTHL: { brief:'Exchange HL with top of stack', flags:'—', bytes:1, cycles:'18', desc:'Swaps H with (SP+1) and L with (SP) without changing SP.', ex:'XTHL' },
  SPHL: { brief:'Copy HL into SP', flags:'—', bytes:1, cycles:'5', desc:'Loads the stack pointer with the content of HL.', ex:'SPHL          ; SP ← HL' },
  PCHL: { brief:'Jump indirect through HL', flags:'—', bytes:1, cycles:'5', desc:'Loads the program counter with HL — an indirect/computed jump.', ex:'PCHL          ; PC ← HL' },
  // Arithmetic
  ...
  INX:  { brief:'Increment register pair by 1', flags:'—', bytes:1, cycles:'5', desc:'rp ← rp + 1. No flags are affected (including CY).', ex:'INX H         ; HL ← HL + 1' },
  DCX:  { brief:'Decrement register pair by 1', flags:'—', bytes:1, cycles:'5', desc:'rp ← rp − 1. No flags are affected.', ex:'DCX D         ; DE ← DE − 1' },
  ...
  EQU:     { brief:'Equate a symbol to a constant value', flags:'—', bytes:0, cycles:'—', desc:'Assigns a constant numeric value to a label. The label can then be used in place of the number throughout the program. This does not emit any machine code.', ex:'MAX EQU 0FFH\nMVI A, MAX    ; assembled as MVI A, 0FFH' },
  DB:      { brief:'Define Byte(s) in memory', flags:'—', bytes:'1+', cycles:'—', desc:'Allocates and initializes one or more bytes of memory. You can provide a comma-separated list of 8-bit values or ASCII strings.', ex:'DB 05H, 0AH   ; allocates two bytes\nDB "Hello"    ; allocates 5 bytes' },
  DW:      { brief:'Define Word(s) in memory', flags:'—', bytes:'2+', cycles:'—', desc:'Allocates and initializes one or more 16-bit words of memory in little-endian order (low byte first).', ex:'DW 1234H, 0BEEFH' },
  DS:      { brief:'Define Storage (reserve memory)', flags:'—', bytes:'0', cycles:'—', desc:'Reserves a specified number of zero-filled bytes in memory. The assembler advances the current address pointer by the given count.', ex:'DS 10H        ; reserve 16 bytes of memory' },
  ...
  DSUB: { brief:'Double subtract (HL = HL - BC)', flags:'CY', bytes:1, cycles:'10', desc:'Undocumented. Subtracts BC from HL. CY is set if a borrow occurs.', ex:'DSUB' },
  ARHL: { brief:'Arithmetic shift right HL', flags:'CY', bytes:1, cycles:'7', desc:'Undocumented. Shifts HL right by 1. MSB is duplicated, LSB goes to CY.', ex:'ARHL' },
  RDEL: { brief:'Rotate DE left through carry', flags:'CY', bytes:1, cycles:'10', desc:'Undocumented. Rotates the 16-bit DE register pair left through the carry flag.', ex:'RDEL' },
  LDHI: { brief:'Load DE with HL + imm8', flags:'—', bytes:2, cycles:'10', desc:'Undocumented. Adds an 8-bit immediate offset to HL and stores the result in DE.', ex:'LDHI 05H' },
  LDSI: { brief:'Load DE with SP + imm8', flags:'—', bytes:2, cycles:'10', desc:'Undocumented. Adds an 8-bit immediate offset to SP and stores the result in DE.', ex:'LDSI 10H' },
  SHLX: { brief:'Store HL to address in DE', flags:'—', bytes:1, cycles:'10', desc:'Undocumented. Stores L at the address in DE, and H at DE+1.', ex:'SHLX' },
  LHLX: { brief:'Load HL from address in DE', flags:'—', bytes:1, cycles:'10', desc:'Undocumented. Loads L from the address in DE, and H from DE+1.', ex:'LHLX' },
  RSTV: { brief:'Restart on overflow', flags:'—', bytes:1, cycles:'12', desc:'Undocumented. Jumps to address 0040H if the undocumented V flag (overflow) is set.', ex:'RSTV' },
  JK:   { brief:'Jump on undocumented K flag', flags:'—', bytes:3, cycles:'10', desc:'Undocumented. Jumps to the specified address if the K flag (bit 5) is set.', ex:'JK 0200H' },
}
